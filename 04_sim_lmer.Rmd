...
```{block, type="info"}
If you already have [pilot data](/data/pilot_data.csv), you can estimate a realistic SD for the subjects' random intercepts using the following code.
```
...
```{block, type="info"}
If you want to simulate data using the exact stimuli you already have pilot data for, you can calculate their random intercepts using the following code and use that instead of generating a random sample with the same SD.</p>
```
...
* The SD for `stim_id` should be near the `stim_sd` of `r stim_sd`. 
...
```{block, type="info"}
[Keep It Maximal](http://talklab.psy.gla.ac.uk/KeepItMaximalR2.pdf) is a great paper on how and why to set random slopes *maximally* like this.
```
...
```{r rslope-mod-varcor2, echo = FALSE}
mod.sum$varcor
```
...
```{r rslope-sim-lmer-default, eval=FALSE}
sim_lmer()
```

Try changing some variables to simulate null effects.

```{r rslope-sim-lmer-null, eval=FALSE}
sim_lmer(hard_congr = 0,
         hard_incon = 0,
         easy_congr = 0,
         easy_incon = 0)
